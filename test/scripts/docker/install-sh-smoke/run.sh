...
SMOKE_PREVIOUS_VERSION="${MILAIDY_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MILAIDY_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="milaidy"
PACKAGE_NAME="${MILAIDY_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MILAIDY_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MILAIDY_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MILAIDY_INSTALL_LATEST_OUT:-}"
fi
